...
import org.example.k_market.repository.CuponRepository;
...
public class CuponDAO {
    ...
    public CuponDTO save(CuponDTO dto) {
        // dto가 null이면 NullPointerException 발생 가능: Service에서 null 체크 후 호출하는 것을 권장
        Cupon entity = dto.toEntity();
        Cupon savedEntity = cuponRepository.save(entity);
        return savedEntity.toDTO();
    }

    public Optional<CuponDTO> findById(Long cuponNo) {
        // PK 타입이 엔티티의 @Id 타입과 다르면 컴파일 오류 발생
        return cuponRepository.findById(cuponNo)
                .map(Cupon::toDTO);
    }
    ...
    public void deleteById(Long cuponNo) {
        // 존재하지 않는 PK 삭제를 막기 위해 existsById로 먼저 확인
        if (cuponRepository.existsById(cuponNo)) {
            cuponRepository.deleteById(cuponNo);
        }
    }
}
