package org.example.k_market.dao;

import org.example.k_market.dto.CouponDTO;
import org.example.k_market.entity.Coupon;
import org.example.k_market.repository.CouponRepository;
import lombok.RequiredArgsConstructor;
import org.springframework.stereotype.Repository;

import java.util.List;
import java.util.Optional;

@RequiredArgsConstructor
@Repository
public class CouponDAO {

    private final CouponRepository couponRepository;

    public CouponDTO save(CouponDTO dto) {
        // dto가 null이면 NullPointerException 발생 가능: Service에서 null 체크 후 호출하는 것을 권장
        Coupon entity = dto.toEntity();
        Coupon savedEntity = couponRepository.save(entity);
        return savedEntity.toDTO();
    }

    public Optional<CouponDTO> findById(Long cuponNo) {
        // PK 타입이 엔티티의 @Id 타입과 다르면 컴파일 오류 발생
        return couponRepository.findById(cuponNo)
                .map(Coupon::toDTO);
    }

    public List<CouponDTO> findAll() {
        return couponRepository.findAll()
                .stream()
                // Entity 내부 연관관계가 LAZY이면 toDTO() 호출 시점에 LazyInitializationException 가능
                .map(Coupon::toDTO)
                .toList();
    }

    public void deleteById(Long cuponNo) {
        // 존재하지 않는 PK 삭제를 막기 위해 existsById로 먼저 확인
        if (couponRepository.existsById(cuponNo)) {
            couponRepository.deleteById(cuponNo);
        }
    }
}
